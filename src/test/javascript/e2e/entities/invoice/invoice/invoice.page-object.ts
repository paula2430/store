...
export class InvoiceUpdatePage {
  ...
  productOrderIdInput = element(by.id('field_productOrderId'));
  ...
  async setProductOrderIdInput(productOrderId: string): Promise<void> {
    await this.productOrderIdInput.sendKeys(productOrderId);
  }

  async getProductOrderIdInput(): Promise<string> {
    return await this.productOrderIdInput.getAttribute('value');
  }
